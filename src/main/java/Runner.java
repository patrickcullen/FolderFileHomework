...
public class Runner {
    ...
    public static void main(String[] args) {
        ...
        File foundFiles = DBHelper.find(File.class, file4.getId());

        Folder foundFolder = DBHelper.find(Folder.class, folder2.getId());

        List<File> hrFiles = DBFolder.getFilesForFolder(foundFolder);

        file2.setExtension(".ppl");
        DBHelper.save(file2);

        File foundaltered = DBHelper.find(File.class, file2.getId());

        DBHelper.delete(file4);

        List<File> filesfewer = DBHelper.getAll(File.class);
    }
}
